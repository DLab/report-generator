...
def plot_anexo_comunas(DataFrame, report_day):
    ...
    to_short_dict = { 'Aysén del Gral. C. Ibáñez del Campo': 'Aysén', 'Magallanes y Antártica Chilena':'Magallanes', 'Metropolitana de Santiago':'Metropolitana',
    		  "Lib. Gral. Bernardo O'Higgins":'O’Higgins', 'La Araucanía':'Araucanía'}
    ...
    for r, region in enumerate(regiones_):
        ...
        for p, page in enumerate(range(int(total_pages))):
            ...
            while i < 8 : #Wait for 8 figures
                ...
                ax.fill_between(datum.index[1:],
                             datum['Low_95'][1:], datum['High_95'][1:], alpha=.4)
                ax.set_title(comuna)
    ...
    folder = 'Anexo/'
    Anexos = []
    keys = [regiones for regiones in filenames]
    pdfs = []
    for key in keys:
        pdfs.append(filenames[key])
    ...
    for item in pdfs:
        ...
        Anexos.append(name+'_'+day+'_'+month+".pdf")
...
if __name__ == '__main__':
    R_e_comunas = pd.read_csv('time_series/R_Efectivo_comunas_'+report_day.replace('/','_')+'.csv')
    plot_anexo_comunas(R_e_comunas, report_day)
